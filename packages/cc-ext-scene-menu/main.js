...
function addNodeWithComponent(name) {
    let param = { name: name };
    Object.assign(param, _paramCache);

    Editor.Scene.callSceneScript('cc-ext-scene-menu', 'add-node-with-component', param, (err) => {
        if (err)
            Editor.log(err);
    });
}

function addPrefab(uuid) {
    let param = {uuid: uuid};
    Object.assign(param, _paramCache);
    Editor.Scene.callSceneScript('cc-ext-scene-menu', 'add-prefab', param, (err) => {
        if (err)
            Editor.log(err);
    });
}
...
function loadMenu() {
    ...
    fs.readFile(configPath, function(err, data) {
        if (err) {
            Editor.error("read cc-ext-scene-menu config fail")
            return;
        }
        ...
        try {
            ...
            Editor.error("parse cc-ext-scene-menu config fail", err)
