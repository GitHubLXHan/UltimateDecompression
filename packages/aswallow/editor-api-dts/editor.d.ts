/**
 * Creator编辑器接口类型声明
 * 基于陈皮皮的改造 @see https://gitee.com/ifaswind/eazax-ccc/blob/master/declarations/editor.d.ts
 */
declare namespace Editor {
    ...
    let appPath: string
    /**版本号 */
    let versions: { CocosCreator: string, 'editor-framework': string, 'asset-db': string, cocos2d: string }
    let projectPath: string
    let _projectProfile: any
    let sceneList: any[]
    let ProjectCompiler: any;
    let Compiler: any;
    let App: {
        /**
         * resources/app 路径
    ...
    let serialize: any;
    let PreviewServer: { start: Function, stop: Function, previewPort: number }
    let _buildCommand: any;
    let _compileCommand: any;
    /**暂存场景 */
    let stashedScene: any;
    /**显示提示 */
    export function T(info: string): void;
    let Profile: any;
    let currentSceneUuid: string
    ...
    namespace MainProcess {
         ...
         */
        class AssetDB {
            _uuidToImportPathNoExt(uuid: string): string;
            _fspath(path: string): string;
...
declare module Editor.Ipc {
    export function sendToMainWin(message: string, ...args): void;
    /**
     * 
     * @param message 
     * @param args 
     */
    export function sendToWins(message: string, ...args);
    /**
...
declare interface BuildOptions {
    ...
    packageName: string;
    platform: "android" | "web-desktop" | "web-mobile" | "mini-game" | "runtime" | "ios";
