...
import { UdMutexZone, UdLayerKind } from "../../extension/view/types/UdLayerKind";
import { UdFullView } from "../view/compoment/UdFullView";
import { UdModuleKind } from "../view/med/UdModuleKind";
import { UdModuleMap } from "../view/med/UdModuleMap";
...
@UdBindMeta
export class UdPanelHub extends UdPanelHubCore {
	private static _Ins: UdPanelHub;

	/**
	 * static Instance:ViewMgr
	 */
	public static get Ins(): UdPanelHub {
		if (this._Ins == null) {
			this._Ins = new UdPanelHub();
		}
		return this._Ins;
	}
	...
	public open<T extends UdViewCore>(classInfo: { new(): T } | string, layerType: UdLayerKind, viewParams?: any, isPlayAnimation: boolean = true): UdViewCore | null {
		// 打开战斗界面时先隐藏其他界面
		// if (layerType === UdLayerKind.Fight) {
	...
	public close<T extends UdViewCore>(classInfo: { new(): T } | string, isDestroy: boolean = false, closeAbove = false) {
		let className = ""
		if (typeof classInfo == "string") {
			className = classInfo
		} else {
	...
	public getTopView(layerType: UdLayerKind): UdViewCore {
	 ...
	 */
	public closeTopView() {
		let view = this.getTopView(UdLayerKind.Panel);
		if (view != null) {
			this.closeByName(view.name);
		}
	}

	public updateView<T extends UdViewCore>(classInfo: { new(): T } | string, args?: any) {
	 ...
	 */
	public addMutuallyExclusive(groupId: UdMutexZone, exclusive: ({ new(): UdFullView } | string)[], priority: number = 0) {
		...
		exclusive.forEach(e => {
			...
			if (typeof e == "string") {
				...
				name = UdReflectKit.getClassName(e);
			}
	...
	/**
		 ...
		 * @param {function} exclusive  当exclusive中有任意界面打开时 不打开界面直到所有界面均关闭
		 */
	public removeMutuallyExclusive(groupId: UdMutexZone, exclusive: ({ new(): UdFullView } | string)[]) {
		if (!this._mutuallyExclusiveConfig[groupId]) {
			this._mutuallyExclusiveConfig[groupId] = []
		}
		exclusive.forEach(e => {
	...
	public openViewById(moduleId: UdModuleKind, viewId: number, layerType: UdLayerKind, viewParams?: any, isPlayAnimation: boolean = true): UdViewCore | null {
		let view = UdModuleMap.get(moduleId, viewId)
		if (view) {
	...
	public closeViewById(moduleId: UdModuleKind, viewId: number, isDestroy: boolean = false, closeAbove = false): void {
		let view = UdModuleMap.get(moduleId, viewId)
		if (view) {
	...
	public updateViewById(moduleId: UdModuleKind, viewId: number, args?: any): void {
		let view = UdModuleMap.get(moduleId, viewId)
		if (view) {
	...
	public isOpenById(moduleId: UdModuleKind, viewId: number) {
		let view = UdModuleMap.get(moduleId, viewId)
		return this._instanceStore.contains(UdReflectKit.getClassName(view));
	}
}
