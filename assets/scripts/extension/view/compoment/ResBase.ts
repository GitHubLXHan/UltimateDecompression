...
export class ResBase {
    ...
    private _componentStore: Dictionary<string, cc.Component> = new Dictionary<string, cc.Component>();
    ...
    public getNode(childName: string): cc.Node | undefined{
        ...
        let node = ResBase.getNode(this._root, childName);
    ...
    public static getNode(node: cc.Node, childName: string): cc.Node | undefined {
        ...
        for (let i = 0; i < len; i++) {
            let child = ResBase.getNode(node.children[i], childName);
        ...
        if (node != null) {
            ...
            if (node.children != null && node.children.length > 0) {
                for (let i = 0; i < node.children.length; i++) {
                    return ResBase.getComponent(node.children[i], type);
                }
            }
        }
