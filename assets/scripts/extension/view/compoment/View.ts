...
@RefClass
export abstract class View extends BaseLog {
	...
	//动画对象数量
	private _animations: List<ViewAnimation>;
	//动画回调
	...
	public loadSkin(args: any[], callback: (name: string, error?: boolean) => void, isPlayAnimation: boolean, loadDoneCB?: (name: string) => void) {
		...
		this._loadId = resource.ResourceManager.sInstance.load(this.prefabPath, cc.Prefab, (err: Error, prefab: cc.Prefab) => {
			...
			this._prefab = prefab;
			resource.ResourceManager.sInstance.cacheAsset(prefab);
	...
	protected init(root: cc.Node, needFullScreen = true) {
		...
		if (this._resBase == null) this._resBase = new ResBase();

		this._resBase.init(this._root);

		let animations = this._root.getComponentsInChildren(ViewAnimation);
		if (animations != null && animations.length > 0) {
			if (this._animations == null) {
				this._animations = new List<ViewAnimation>();
			} else {
				this._animations.clear();
			}
			animations.forEach((value: ViewAnimation, index: number) => {
		...
		if (needFullScreen) {
			...
			if (widget) {
				if (DeviceUtils.Ins.IsXyx) {
					if (DeviceUtils.Ins.statusBarHeight <= 0) {
						//windows没有刘海 不需要适配
						widget.top = 0
					} else {
						widget.top = this.ignoreWidgetTopAdjust ? -DeviceUtils.Ins.statusBarHeight : - DeviceUtils.Ins.xyxBarHeight + 30;
					}
				} else {
					widget.top = this.ignoreWidgetTopAdjust ? -DeviceUtils.Ins.statusBarHeight : 0;
				}

				widget.bottom = this.ignoreWidgetBottomAdjust ? -DeviceUtils.Ins.statusBottomHeight : 0;
	...
	/**
	 * @description: 所在bundle包。默认resource
	 */
	...
	public playCloseAnimation(callback: (name: string) => void, isSkip: boolean, isDestroy: boolean = false, isPlayCloseSound: boolean = true) {
		...
		if (this._animations != null && this._animations.length > 0) {
			let max = this._animations.length;
			for (let i = 0; i < max; i++) {
				let animation: ViewAnimation = this._animations.get(i);
	...
	public destroy() {
		...
		resource.ResourceManager.sInstance.uncacheAsset(this._prefab);
