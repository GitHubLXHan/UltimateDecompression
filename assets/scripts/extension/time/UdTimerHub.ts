...
import { IUdTickable } from "../update/IUdTickable";
import { UdTickHub } from "../update/UdTickHub";
import { UdTimerSlot } from "./UdTimerSlot";

export class UdTimerHub implements IUdTickable
{
    private static _ins:UdTimerHub;
    /**
     * static Instance:TimerHeap
     */
    public static get Ins():UdTimerHub 
    {
        if (this._ins == null){
            this._ins = new UdTimerHub();
        }
    ...
    private constructor()
    {
        UdTickHub.Ins.addUpdateHandler(this);
    }
    private _timers:UdSeqList<UdTimerSlot> = new UdSeqList<UdTimerSlot>();
     ...
     * @param {type} 
     */    
    private addTimer(tData:UdTimerSlot)
    {
        this._timers.add(tData);
        // UdLogHub.log(`添加计时器${tData.id}，${tData.nextTrick - this._timestamp}秒后`);
    ...
    public onUpdate(deltaTime:number)
    {
        ...
        let timeScale = UdTickHub.Ins.timeScale;
        ...
        while (this._timers.length > 0 && this._timestamp >= this._timers[0].nextTrick) {
            ...
            if (trickData.interval > 0) {
                ...
                trickData.doAction();
                // UdLogHub.log(`计时器结束${trickData.id}，${trickData.nextTrick - this._timestamp}`);
                UdObjCache.Ins.recover(trickData);
            }
        }
    ...
    public dispose()
    {
        if (this._timers.length > 0)
        {
            ...
            {
                let tData = this._timers.get(i);
                tData && UdObjCache.Ins.recover(tData);
            }
     ...
     * @param {type} 
     */
    private getTimerData(delay:number, interval:number, handler:(args:any[])=>void, args:any[]):UdTimerSlot
    {
        let tData = UdObjCache.Ins.impl(UdTimerSlot);
        tData.id = ++this._id;
     ...
     * @param {type} 
     */
    private sortPriority(a:UdTimerSlot, b:UdTimerSlot)
    {
        if (a == null || b == null)
            return 0
