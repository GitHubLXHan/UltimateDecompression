...
export class UdRandomKit
{
     ...
     * @return: 
     */
    public static getRandomListElm<T>(list:UdSeqList<T>):T {
        if (list == null || list.length <= 0)
            return null;

        let index = UdRandomKit.getRandomInt(0, list.length - 1);
        return list[index];
    }
    ...
    public static randomSort(a:object, b:object):number {
        return UdRandomKit.getRandomInt(-1, 1);
    }
