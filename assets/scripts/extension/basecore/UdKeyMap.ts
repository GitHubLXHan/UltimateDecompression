import { UdSeqList } from "./UdSeqList";
import { UdBindMeta } from "./UdDecoratorKit";
import { UdLogHub } from "../log/UdLogHub";

@UdBindMeta
export class UdKeyMap<T extends string | number, U> {
    private _keyList: UdSeqList<T> = new UdSeqList<T>();
    private _valueList: UdSeqList<U> = new UdSeqList<U>();
    //只读，不允许修改
     ...
     * @param {type} 
     */
    public add(key: T, value: U):UdKeyMap<T, U> {
     ...
     * @param dic
     */
    public addRange(dic: UdKeyMap<T, U>):UdKeyMap<T, U> {
        if (this._isReadonly || dic == null) return this;
        if (this._isIterating) {
            UdLogHub.logError("字典在迭代过程中不允许做增/删操作。");
            return null;
        }
     ...
     * @param value
     */
    public setValue(key: T, value: U):UdKeyMap<T, U> {
        return this.overlay(key, value);
    }
     ...
     * @param {type} 
     */
    public overlay(key: T, value: U):UdKeyMap<T, U>  {
     ...
     */
    public clear():UdKeyMap<T, U> {
        if (this._isReadonly) return this;
        if (this._isIterating) {
            UdLogHub.logWarning("字典在迭代过程中不允许做增/删操作。");
            return;
        }
     ...
     * @param {boolean} value
     */    
     public setReadonly(value:boolean):UdKeyMap<T, U> {
        if (!this._isReadonly) {
     ...
     * @param 是否复制
     */
    public getKeys(isClone: boolean = true): UdSeqList<T> {
        if (isClone) {
     ...
     * @param 是否复制
     */
    public getValues(isClone: boolean = true): UdSeqList<U> {
        if (isClone) {
     ...
     * @description: 克隆数据
     */
    public clone(): UdKeyMap<T, U> {
        let dic = new UdKeyMap<T, U>();
        if (this.length > 0) {
            dic.addRange(this);
        }
