...
@UdBindMeta
export class UdAudioTrack extends UdSignalBus<UdAudioSignal, UdAudioTrack> implements IUdReusable {
    ...
    private _fadeTween?: cc.Tween<UdAudioTrack>;
    ...
    public preload(path: string) {
        ...
        this._path = path;
        this._loadId = udRes.UdResHub.sInstance.load(this._path, cc.AudioClip, (err: Error, clip: cc.AudioClip) => {
            this._loadId = 0;
            if (err != null) {
                this.dispatchEvent(UdAudioSignal.TrackError);
                return;
            }

            udRes.UdResHub.sInstance.cacheAsset(clip);
            this._clip = clip;
            this.loadCompleted();
        })
    }
    ...
    public play(volume: number = 1, loop: boolean = true, fadeTime: number = 0) {
        ...
        this._volume = volume;
        this._isLoop = loop;
        this._fadeTime = UdMathKit.clamp(fadeTime, 0, 10);
    ...
    private loadCompleted() {
        ...
        this.dispatchEvent(UdAudioSignal.TrackReady);
    }
     ...
     */
    private playHandler() {
        this.dispatchEvent(UdAudioSignal.TrackEnd);
    }
    ...
    public recover() {
        ...
        if (this._clip != null) {
            udRes.UdResHub.sInstance.uncacheAsset(this._clip);
