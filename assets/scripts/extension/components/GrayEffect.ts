...
import { GameSpine } from "../game/GameSpine";
...
@ccclass
@menu("UI/GrayEffect(灰度效果)")
@executeInEditMode
export class GrayEffect extends cc.Component {

    private _spriteList: List<cc.Sprite> = new List<cc.Sprite>();
    private _labelList: List<cc.Label> = new List<cc.Label>();
    private _spineList: List<sp.Skeleton> = new List<sp.Skeleton>();
    ...
    public set graySpineMaterial(value: cc.Material) {
        if (this._graySpineMaterial) {
            resource.ResourceManager.sInstance.uncacheAsset(this._graySpineMaterial);
        }
        this._graySpineMaterial = value;
        if (this._graySpineMaterial) {
            resource.ResourceManager.sInstance.cacheAsset(this._graySpineMaterial);
        }
    }
    ...
    public onLoad() {
        ...
        {
            let sprites = this.node.getComponentsInChildren(GameSprite);
            if (sprites.length > 0) {
                for (let i = 0; i < sprites.length; i++) {
                    let sprite = sprites[i];
                    this._spriteList.add(sprite);
        ...
        {
            let spines = this.node.getComponentsInChildren(GameSpine);
            if (spines.length > 0) {
                for (let i = 0; i < spines.length; i++) {
                    let spine = spines[i];
                    this._spineList.add(spine);
    ...
    public onEnable() {
        ...
        if (this.graySpineMaterial) {
            ...
            if (CC_EDITOR) {
                ...
                this._loader = resource.ResourceManager.sInstance.load(path, cc.Material, this.loadSpineMaterialFinish.bind(this));
            }
        }
    }
    ...
    public onDisable() {
        if (this._loader) {
            resource.ResourceManager.sInstance.stopLoad(this._loader)
            this._loader = 0
        }
     ...
     * @param isGray 灰度状态 
     */
    private setSpritesGrayState(list: List<cc.Sprite>, isGray: boolean) {
     ...
     * @param isGray 灰度状态 
     */
    private setLabelsGrayState(list: List<cc.Label>, isGray: boolean) {
        if (list && list.length > 0) {
            for (let i = 0; i < list.length; i++) {
     ...
     * @param isGray 灰度状态 
     */
    private setSpinesGrayState(list: List<sp.Skeleton>, isGray: boolean) {
        if (list && list.length > 0) {
            for (let i = 0; i < list.length; i++) {
