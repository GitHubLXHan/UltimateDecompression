import { UdFilePathKit } from "../../core/utils/UdFilePathKit";
...
import { UdAssetItem } from "./UdAssetItem";
import { UdLoadRequest } from "./UdLoadRequest";
import { UdResSignal } from "./UdResSignal";
import { UdLoadRunner } from "./UdLoadRunner";
import { UdLoadTier } from "./UdLoadTier";

export module udRes {
	export class UdAssetUtil {
	...
	export class UdResHub extends UdSignalBus<UdResSignal, UdResHub>  {
		// 外部资源文件夹名称
		...
		private _loader: UdLoadRunner;
		private _gcTimer;

		constructor() {
			super();
			window["_a_r"] = () => {
				return UdResHub.sInstance;
			};
			this._loader = new UdLoadRunner();
		}
		...
		private static _sInstance: UdResHub;

		public static get sInstance(): UdResHub {
			if (this._sInstance == null) {
				this._sInstance = new UdResHub();
			}
		...
		private get loader(): UdLoadRunner {
			return this._loader;
		}

		/**全部动态资源缓存 */
		private _dynamicAssets: UdKeyMap<string, UdAssetItem> = new UdKeyMap();
		// 不能单独存储可释放列表，因为引擎底层销毁节点时减静态引用时我们不知道，除服hook资源减引用函数，但暂时没必要
		// private _unuseDynamicAssets: UdKeyMap<string, UdAssetItem> = new UdKeyMap();
			...
			for (let i = this._dynamicAssets.length - 1; i >= 0; --i) {
				...
				if (ccAsset.asset.refCount == 1) {
					if (now - ccAsset.lastUseTimeStamp > ccAsset.cacheTiem) {
						...
						let depRes: string[] = asset[UdResHub.ExtDepResKey];
						if (depRes) {
							...
							asset[UdResHub.ExtDepResKey] = undefined;
						}
						this.uncacheAsset(asset);
						UdObjCache.Ins.recover(ccAsset);
					}
				}
			}
		}
		...
		public addDynamicAsset(asset: cc.Asset): void {
			let key = asset[UdResHub.UuidKey];
			if (!this._dynamicAssets.contains(key)) {
				let ccAsset = UdObjCache.Ins.impl(UdAssetItem);
		...
		public releaseDynamicAsset(asset: cc.Asset): void {
			let dynamicAsset = this._dynamicAssets.getValue(asset[UdResHub.UuidKey]);
			if (dynamicAsset?.asset?.refCount == 1) {
				this._dynamicAssets.remove(dynamicAsset.key);
				this.uncacheAsset(asset);
				UdObjCache.Ins.recover(dynamicAsset);
			}
		}

		/**判断对应任务是否在加载 */
		public isLoading(id: number): boolean {
			return UdResHub.sInstance.loader.isTaskLoading(id);
		}

		/**停止加载对应任务 */
		public stopLoad(id: number): void {
			UdResHub.sInstance.loader.stopLoadTask(id);
		}
		...
		public cacheAsset(asset: cc.Asset | cc.Asset[]): void {
			if (asset) {
				...
				for (let tempAsset of assetList) {
					tempAsset.addRef();
					this._dynamicAssets.getValue(tempAsset[UdResHub.UuidKey])?.updateTimeStamp();
				}
			}
		}
		...
		public isExtRes(path: string) {
			...
			for (let i = 0; i < UdResHub._extResPathPrefixList.length; i++) {
				if (path.startsWith(UdResHub._extResPathPrefixList[i])) {
					return true;
				}
			}
			return false;
		...
		public getAsset<T extends cc.Asset>(path: string, type: typeof cc.Asset): T | undefined {
			...
			if (bundleName) {
				...
				let uuid = resPath + (type == cc.SpriteFrame ? UdResHub.SpriteFrameSuffix : '');
		...
		private _getLoadTask(priority = UdLoadTier.STANDARD, loadFunc: Function, loadFuncTarget: any, onComplete?: Function, ...args: any[]): UdLoadRequest {
			let loadTask = UdResHub.sInstance.loader.fetchLoadTask();
			let taskId = loadTask.id;
			loadTask.priority = priority ? priority : UdLoadTier.STANDARD;
		 ...
		 * @returns
		 */
		public load<T extends cc.Asset>(path: string, type: typeof cc.Asset, onComplete?: (err: Error | null, data: T | null) => void, priority?: UdLoadTier): number {
			...
			// 		UdResHub.sInstance.loader.scheduleNextTask(loadTask);
			// 		return loadTask.id;
			// 	}
			// }else{
			let { bundleName, resPath } = this.parseResPath(path);
				...
				window.aswallow.extAssetMgr.load(
					...
					(err, asset: cc.Asset) => {
						if (!err) {
							...
							console.warn(err.message);
							this.dispatchEvent(UdResSignal.AssetLoadFail, "res loader error", `path:${path} msg:${err.message}`)
						}
		 ...
		 * @returns
		 */
		public loadList<T extends cc.Asset>(paths: string[], type: typeof cc.Asset, onComplete?: (err: Error | null, data: T[] | null) => void, priority?: UdLoadTier): number {
		...
		/**加载SpriteFrame */
		public loadSpriteFrame(path: string, onComplete?: (err: Error | null, data: cc.SpriteFrame | null) => void, priority?: UdLoadTier): number {
			return this.load(path, cc.SpriteFrame, onComplete, priority);
		}

		/**加载spine */
		public loadSpine(path: string, onComplete?: (err: Error | null, data: sp.SkeletonData | null) => void, priority?: UdLoadTier): number {
			if (!this.isExtRes(path)) {
				...
				// 			UdResHub.sInstance.loader.onLoadTaskDone(loadTaskId);
				// 		},
				// 		null,
				// 		extraData
				// 	);
		...
		/**加载二进制数据 */
		public loadBytes(path: string, onComplete?: (err: Error | null, data: Uint8Array | null) => void, priority?: UdLoadTier): number {
			return this.load(path, cc.Asset, (err, data) => {
				if (!err) {
                    let bytes = UdAssetUtil.getBytes(data);
                    if (bytes) {
		...
		/**加载远程资源 */
		public loadRemote(path: string, options: Record<string, any>, onComplete?: (err: Error | null, data: cc.Asset | null) => void, priority?: UdLoadTier): number {
			if (typeof path !== 'string') {
		...
		private _getSpriteFrame(asset: cc.Texture2D): cc.SpriteFrame {
			...
			let cachePath = asset[UdResHub.UuidKey] + UdResHub.SpriteFrameSuffix;
			...
			if (cacheAsset) {
				...
				// 兼容引擎原生引用计数逻辑
				spriteFrameAsset[UdResHub.UuidKey] = cachePath;
				// 记录直接引用资源，释放对应图片时再-1
				spriteFrameAsset[UdResHub.ExtDepResKey] = [asset[UdResHub.UuidKey]];
		...
		public parseResPath(source: string): { bundleName: string; resPath: string; ext: string } {
			...
			ext =  UdFilePathKit.splitext(resPath)[1];
			resPath = UdFilePathKit.changeExtname(resPath, "");
