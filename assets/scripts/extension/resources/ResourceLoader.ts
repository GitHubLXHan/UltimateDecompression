...
export class ResourceLoader {
	...
	private _loadingQueue: List<LoadTask> = new List();
	private _waitingQueue: List<LoadTask> = new List();
	private _loadOffList: List<number> = new List<number>();
	...
	constructor() {
		...
		// /**
		//  * 拦截加载资源管道
		//  * 每个加载任务在 options 中都带有 LoadTask 的 id
	...
	private _checkAndstopTask(task: cc.AssetManager.Task, loadOffList: List<number>, done?: (err: Error) => void) {
	...
	/**获取一个加载任务实例 */
	public fetchLoadTask(): LoadTask {
		let loadTask = PoolMgr.Ins.impl(LoadTask);
	...
	/**开始下一个任务 */
	public scheduleNextTask(loadTask?: LoadTask) {
		if (loadTask) {
			this._waitingQueue.push(loadTask);
		}
		if (this._loadingQueue.length >= this.maxRequestCountSimultaneously) {
			if (!loadTask || loadTask.priority < ResourceLoadPriority.GOD)
				// GOD可以插队
				return;
		}
		// 由大到小排列
		this._waitingQueue.sort((a: LoadTask, b: LoadTask) => {
