...
export const UD_SPINE_READY = "loadComplete";

export const UD_SPINE_LOW_PATH = "0.24/";
export const UD_SPINE_HIGH_PATH = "1/";


/**模型品质等级 */
export const UD_MODEL_TIER = cc.Enum({
	/**普通 */
	NORMAL: 1,
	/**低模 */
	LOW: 2,
...
export class UdSpine extends sp.Skeleton {
	private static PrefixUrl: string = "http://localhost:7456/ext-res/";

	private _loadIdList: UdSeqList<number> = new UdSeqList<number>();
	...
	@property
	protected _qualityLevel: number = UD_MODEL_TIER.STANDARD;
	...
	private _tempAssetList = new UdKeyMap<string, cc.Asset>();
	...
	public onLoad() {
		if (this.skeletonData) {
			udRes.UdResHub.sInstance.cacheAsset(this.skeletonData);
		}
	...
	public play(action: string, isLoop: boolean = false, reset: boolean = false, mix: number = 0) {
		...
		if (reset || this._action != action || this._isLoop != isLoop) {
			...
			if (!this.findAnimation(this._action)) {
				UdLogHub.log(`模型${this._source}找不到对应动画${this._action}`);
				return;
			}
			if (mix > 0 && oldAction) {
	...
	private updateModel() {
		...
		let resourceManagerIns = udRes.UdResHub.sInstance;
		if (resourceManagerIns.isExtRes(this._source)) {
			let preFix = CC_EDITOR ? UdSpine.PrefixUrl : "";
			let loadFunc = CC_EDITOR ? cc.assetManager.loadRemote.bind(cc.assetManager) : resourceManagerIns.load.bind(resourceManagerIns);
			let logFunc = CC_EDITOR ? Editor.error.bind(Editor) : UdLogHub.logError.bind(UdLogHub);
			...
			if (this._qualityLevel == UD_MODEL_TIER.STANDARD) {
				if (CC_EDITOR) {
					...
					let loadId = resourceManagerIns.loadSpine(this._source, (err, asset) => {
						...
						if (!(asset instanceof sp.SkeletonData)) {
							...
							UdLogHub.logError(`spine加载失败，可能是缺少资源导致：`, this._source);
							cc.assetManager.assets.remove(asset[udRes.UdResHub.UuidKey]);
							return;
						}
				...
				UdLogHub.logError(`非NORMAL模式只支持外部路径加载`);
			}
		}
	}
	...
	private _getQualityRes(prefixStr = ""): string[] {
		...
		if (this._qualityLevel == UD_MODEL_TIER.STANDARD) {
			...
			let qualityRoot = this._qualityLevel == UD_MODEL_TIER.ECONOMY ? UD_SPINE_LOW_PATH : UD_SPINE_HIGH_PATH;
			// @ts-ignore
			result.push(cc.path.join(prefixStr, this._source));
			//@ts-ignore
		...
		suffix.forEach((ext: string, index: number) => {
			result[index] = UdFilePathKit.changeExtname(result[index], ext);
		});
		return result;
	}
	...
	private _loadSpineRes(spineRes: string[], loadFunc: (path: string, type: typeof cc.Asset, onComplete?: (err: Error | null, data: cc.Asset | null) => void) => any, logFunc?: Function) {
		...
		if (!udRes.UdResHub.sInstance.isExtRes(spineRes[0].replace(UdSpine.PrefixUrl, ""))) {
		...
		let loadComplete = () => {
			...
			// let UuidKey = udRes.UdResHub.UuidKey;
			// spineAsset[UuidKey] = uuid;
			// spineAsset[udRes.UdResHub.ExtDepResKey] = [];
			...
			// 	spineAsset[udRes.UdResHub.ExtDepResKey].push(textureAsset[UuidKey]);
			// }
			...
			// 首次加载，加入动态资源缓存
			udRes.UdResHub.sInstance.addDynamicAsset(spineAsset);
			// setSpine(spineAsset);
	...
	private _addTempAsset(path: string, asset: cc.Asset): void {
		...
		udRes.UdResHub.sInstance.cacheAsset(asset);
	}
	...
	private _loadSpineInEditor(spineRes: string[]) {
		...
		let textName = spineRes[1];
		let textureChecked = false;
		let tempAssetList: UdKeyMap<string, cc.Asset> = new UdKeyMap<string, cc.Asset>();

		let loadAny = (path: string, assetType: typeof cc.Asset, cb: (assets: cc.Asset) => void) => {
			let res: { bundleName: string; resPath: string; ext: string } = udRes.UdResHub.sInstance.parseResPath(path);
			...
			res.resPath = UdFilePathKit.changeExtname(res.resPath, res.ext);
	...
	private _createSkeletonData(uuid: string, textAsset: cc.TextAsset, jsonAsset: cc.JsonAsset, spineRes: string[], tempAssetList: UdKeyMap<string, cc.Asset>): sp.SkeletonData {
		let spineAsset = new sp.SkeletonData();
		let UuidKey = udRes.UdResHub.UuidKey;
		spineAsset[UuidKey] = uuid;
		spineAsset[udRes.UdResHub.ExtDepResKey] = [];
	...
	private _updateSkeletonDataAndRefCount(newSk?: sp.SkeletonData) {
		if (this.skeletonData) udRes.UdResHub.sInstance.uncacheAsset(this.skeletonData);
		this.skeletonData = newSk;
		if (this.skeletonData) udRes.UdResHub.sInstance.cacheAsset(this.skeletonData);
	}

	private _stopLoad(): void {
		while (this._loadIdList.length > 0) {
			udRes.UdResHub.sInstance.stopLoad(this._loadIdList.shift());
		}
		this._tempAssetList.foreach((key, asset) => udRes.UdResHub.sInstance.uncacheAsset(asset));
	...
	private _skeletonToUrl() {
		...
		if (skeletonPath.startsWith(udRes.UdResHub.bundlesPrefix)) {
			skeletonPath = skeletonPath.replace(udRes.UdResHub.bundlesPrefix, "");
		}
		if (skeletonPath.startsWith(udRes.UdResHub.resourcesPrefix)) {
