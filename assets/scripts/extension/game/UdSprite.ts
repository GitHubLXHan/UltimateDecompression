...
export class UdSprite extends cc.Sprite {
	...
	onLoad(): void {
		...
		if (this.spriteFrame) {
			udRes.UdResHub.sInstance.cacheAsset(this.spriteFrame);
		}
		...
		if (this._dirtyFlag) {
			...
			if (CC_EDITOR) {
				...
				if (udRes.UdResHub.sInstance.isExtRes(this._source)) {
					...
					let pathArr = UdFilePathKit.splitext(this._source);
					let pathTmp = pathArr[1] ? this._source : UdFilePathKit.changeExtname(this._source, ".png");
					...
					// 组织路径
					let res: { bundleName: string; resPath: string; ext: string } = udRes.UdResHub.sInstance.parseResPath(this._source);

					let pathArr = UdFilePathKit.splitext(this._source);
					res.resPath = pathArr[1] ? UdFilePathKit.changeExtname(res.resPath, pathArr[1]) : UdFilePathKit.changeExtname(res.resPath, ".png");
					...
					cc.assetManager.loadAny(`${uuid}`, { type: cc.Texture2D, bundle: res.bundleName, requestType__: "uuid" }, (error: Error, assets: cc.Texture2D) => {
						if (error) {
							...
							if (this._resUuid) {
								...
								resUrl = resUrl.replace(udRes.UdResHub.resourcesPrefix, "").replace(udRes.UdResHub.bundlesPrefix, "")
								this.source = resUrl;
								this._resUuid = "";
							}
							return;
						...
						if (subAssetInfo?.length > 0) {
							...
							spTmp[udRes.UdResHub.UuidKey] = subAssetInfo[0].uuid;
							this.curSp = spTmp;
						}
						// 设置九宫格数据
				...
				if (!this._source) {
					...
					if (this._source.indexOf("https://") >= 0 || this._source.indexOf("http://") >= 0 || this._source.indexOf("data:image/") >= 0) {
						...
						if (this._source.indexOf(".php") >= 0 || this._source.indexOf("data:image/") >= 0) {
							...
							this._loaderId = udRes.UdResHub.sInstance.loadRemote(this._source, { ext: ".png" }, this.onComplete.bind(this));
						}
					} else {
						this._loaderId = udRes.UdResHub.sInstance.loadSpriteFrame(this._source, this.onComplete.bind(this));
					}
				}
			}
		}
	...
	public set genMipmaps(value: boolean) {
		...
		if (this._genMipmaps) {
			if (t2d["_texture"] != null) {
				...
				if (!UdMathKit.isPow2(w) || !UdMathKit.isPow2(h)) {
					UdLogHub.logWarning("[UdSprite] 当贴图宽高不是2次幂的时候，设置genMipmaps无效!", w, h);
					return;
				}
			}
		}
	...
	private updateSpriteFrameAndRefCount(newSp?: cc.SpriteFrame) {
		if (this.spriteFrame) udRes.UdResHub.sInstance.uncacheAsset(this.spriteFrame);
		this.spriteFrame = newSp;
		if (this.spriteFrame) udRes.UdResHub.sInstance.cacheAsset(this.spriteFrame);
	}
	...
	private _onSpriteframeChanged() {
		...
		if (this.spriteFrame && this.spriteFrame.getTexture()) {
			...
			if (texturePath && texturePath.startsWith(udRes.UdResHub.bundlesPrefix)) {
				texturePath = texturePath.replace(udRes.UdResHub.bundlesPrefix, "");
			}
			if (texturePath && texturePath.startsWith(udRes.UdResHub.resourcesPrefix)) {
